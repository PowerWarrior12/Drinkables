...
class PropertyAdapter(onRatingClick: RatingView.OnItemClickListener, action: ((String?) -> Unit) -> Unit) : AsyncListDifferDelegationAdapter<PropertyModel>(
    PropertyModelDiffCallback,
    propertyAdapterDelegate(),
    propertyTitleAdapterDelegate(),
    propertyRatingAdapterDelegate(onRatingClick, action),
    propertyIndicatorAdapterDelegate(),
    propertyUserRatingAdapterDelegate()
)